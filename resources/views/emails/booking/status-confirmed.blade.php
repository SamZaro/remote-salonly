<x-layouts.email>
    <x-slot name="preview">
        {{ __('Uw reservering op :date om :time is bevestigd!', ['date' => $booking->booking_date->format('d-m-Y'), 'time' => $booking->booking_time]) }}
    </x-slot>

    <tr>
        <td class="sm-px-6" style="border-radius: 4px; padding: 48px; font-size: 16px; color: #334155; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05)" bgcolor="#ffffff">
            <h1 class="sm-leading-8" style="margin: 0 0 24px; font-size: 24px; font-weight: 600; color: #000">
                {{ __('Uw reservering is bevestigd!') }}
            </h1>
            ...
            <p style="margin: 0 0 24px; line-height: 24px">
                {{ __('Goed nieuws! Uw reservering bij :app_name is bevestigd. Hieronder vindt u de details van uw afspraak.', ['app_name' => config('app.name')]) }}
            </p>
            ...
            <p style="margin: 0 0 24px; line-height: 24px">
                {{ __('Wij kijken ernaar uit u te mogen verwelkomen. Heeft u vragen of wilt u uw reservering wijzigen? Neem dan contact met ons op.') }}
            </p>
