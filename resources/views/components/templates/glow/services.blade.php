{{--
    Glow Template: Services Section
    Warm minimalist — clean grid, no decorative corners or gradient icons
    Props: $content, $theme
--}}
@props([
    'content' => [],
...
<section id="services" class="py-20 lg:py-28" style="background-color: {{ $backgroundColor }};">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {{-- Services grid --}}
        <div class="grid gap-px sm:grid-cols-2 lg:grid-cols-3" style="background-color: {{ $primaryColor }}60; border-radius: 12px; overflow: hidden;">
            @foreach($items as $index => $item)
                <div
                    class="p-8 transition-colors duration-300"
                    style="background-color: {{ $backgroundColor }};"
                    x-data x-intersect.once="$el.style.opacity = 1; $el.style.transform = 'translateY(0)'"
                    x-bind:style="'opacity: 0; transform: translateY(16px); transition: all 0.6s ease-out {{ $index * 0.08 }}s;'"
                >
                    {{-- Icon --}}
                    <div class="w-10 h-10 mb-5 flex items-center justify-center">
                    ...
                    {{-- Title --}}
                    <h3 class="text-xl font-bold mb-2" style="color: {{ $headingColor }}; font-family: '{{ $headingFont }}', sans-serif;">
                        {{ $item['title'] }}
                    </h3>

                    {{-- Description --}}
                    <p class="mb-5 leading-relaxed" style="color: {{ $textColor }};">
                        {{ $item['description'] }}
                    </p>

                    {{-- Price & CTA --}}
                    <div class="flex items-center justify-between">
                        <span class="font-bold" style="color: {{ $secondaryColor }};">
                            {{ $item['price'] }}
                        </span>
                        <a
                            href="#contact"
