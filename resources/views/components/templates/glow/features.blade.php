{{--
    Glow Template: Features Section
    Warm minimalist — cards with left accent border and side-aligned icon
...
@php
    $title = $content['title'] ?? 'Waarom Bij Ons';
...
<section id="features" class="py-20 lg:py-28" style="background-color: {{ $backgroundColor }};">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {{-- Features grid: 2x2 cards with left accent --}}
        <div class="grid gap-6 sm:grid-cols-2">
            @foreach($items as $index => $item)
                <div
                    class="flex gap-6 p-8 lg:p-10"
                    style="background-color: white; border-radius: 12px; border-left: 3px solid {{ $primaryColor }};"
                    x-data x-intersect.once="$el.style.opacity = 1; $el.style.transform = 'translateY(0)'"
                ...
                >
                    <div
                        ...
                        style="background-color: {{ $accentColor }}; border-radius: 10px;"
                    >
