...
>
    ...
    <div class="absolute inset-0">
        @foreach($sliderImages as $index => $media)
            <div class="absolute inset-0 transition-opacity duration-1000" x-show="currentSlide === {{ $index }}" x-transition:enter="transition ease-out duration-1000" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-1000" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
                ...
                @if($overlayOpacity > 0)<div class="absolute inset-0" style="background: linear-gradient(to bottom, rgba(0,0,0,{{ $overlayOpacity }}), rgba(0,0,0,{{ $overlayOpacity * 0.6 }}), rgba(0,0,0,{{ $overlayOpacity }}));"></div>@endif
            </div>
        @endforeach
    </div>
    ...
    <div class="relative z-10 mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 text-center py-20">
        ...
        @if($title)<h1 class="text-5xl sm:text-6xl md:text-7xl font-light tracking-wide mb-6" style="color: {{ $textColor }}; font-family: 'Playfair Display', serif;">{!! $title !!}</h1>@endif
        @if($subtitle)<p class="text-lg sm:text-xl md:text-2xl mb-12 max-w-2xl mx-auto font-light tracking-wide opacity-90" style="color: {{ $textColor }};">{{ $subtitle }}</p>@endif

        <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
            @if($ctaText)
                <a href="{{ $ctaLink }}" class="inline-flex items-center justify-center px-8 py-4 text-lg font-medium transition-all duration-300 hover:scale-105" style="background-color: {{ $primaryColor }}; color: {{ $secondaryColor }}; border-radius: {{ $buttonRadius }};">{{ $ctaText }}</a>
            @endif
            <a href="#services" class="inline-flex items-center justify-center px-8 py-4 text-lg font-medium border-2 transition-all duration-300 hover:bg-white/10" style="border-color: {{ $textColor }}; color: {{ $textColor }}; border-radius: {{ $buttonRadius }};">Onze diensten</a>
        </div>
    </div>
    ...
    @if($showIndicators && $sliderImages->count() > 1)
        <div class="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex gap-3">
            @foreach($sliderImages as $index => $media)
                <button class="w-2 h-2 transition-all duration-300" :class="currentSlide === {{ $index }} ? 'scale-150' : 'opacity-50'" :style="currentSlide === {{ $index }} ? 'background-color: {{ $primaryColor }}' : 'background-color: {{ $textColor }}'" @click="goToSlide({{ $index }})" aria-label="Go to slide {{ $index + 1 }}"></button>
            @endforeach
        </div>
    @endif
...
<section id="slider" class="relative min-h-screen flex items-center justify-center" style="background-color: {{ $secondaryColor }};">
    <div class="text-center px-4">
        @if($title)<h1 class="text-5xl sm:text-6xl font-light mb-6" style="color: {{ $textColor }}; font-family: 'Playfair Display', serif;">{!! $title !!}</h1>@endif
