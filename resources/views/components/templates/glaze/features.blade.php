{{--
    Glaze Template: Features Section
    Dark section with rose-accented feature cards
...
@php
    ...
    $items = $content['items'] ?? [
        ['title' => __('Precision'), 'description' => __('Our nail stylists deliver flawless results through meticulous attention to detail and expert technique.'), 'icon' => 'sparkles'],
        ['title' => __('Hygiene First'), 'description' => __('We maintain the highest standards of cleanliness with sterilised tools and premium products.'), 'icon' => 'heart'],
        ['title' => __('Trendsetting'), 'description' => __('Always ahead of the curve with the latest nail art techniques, colors and styles.'), 'icon' => 'swatch'],
        ['title' => __('Personal Touch'), 'description' => __('Every treatment is tailored to your style, preferences and nail type.'), 'icon' => 'chat'],
    ];
...
<section id="features" class="py-20 lg:py-28" style="background-color: {{ $secondaryColor }}; font-family: '{{ $bodyFont }}', sans-serif;">
        ...
        @if(count($items) > 0)
            <div class="grid gap-6 {{ $gridCols }}">
                @foreach($items as $index => $item)
                    <div
                        class="text-center p-8 rounded-2xl transition-all duration-500 hover:-translate-y-1"
                        style="background-color: {{ $primaryColor }}08; border: 1px solid {{ $primaryColor }}15;"
                        x-data x-intersect.once="$el.style.opacity = 1; $el.style.transform = 'translateY(0)'"
                        style="opacity: 0; transform: translateY(20px); transition: all 0.7s cubic-bezier(0.22, 1, 0.36, 1) {{ $index * 0.1 }}s;"
                    >
                        {{-- Icon --}}
                        <div
                            class="w-14 h-14 rounded-xl flex items-center justify-center mx-auto mb-6"
                        ...
                        <h3 class="text-lg font-bold mb-3" style="color: #ffffff; font-family: '{{ $headingFont }}', sans-serif;">
                            {{ $item['title'] ?? '' }}
                        </h3>
