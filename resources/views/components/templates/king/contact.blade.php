{{--
    King Template: Contact Section
    "Royal Throne" — 3-column info cards, dark accent card, diamond accents
...
@php
    $title = $content['title'] ?? __('Visit the King');
    $subtitle = $content['subtitle'] ?? __('We look forward to welcoming you');
    $address = $content['address'] ?? 'Keizersgracht 123, 1015 CJ Amsterdam';
    $phone = $content['phone'] ?? '020 - 123 4567';
    $email = $content['email'] ?? 'info@kingbarbershop.nl';
...
<section id="contact" class="py-24 lg:py-36" style="background-color: {{ $primaryColor }}04; font-family: '{{ $bodyFont }}', sans-serif;">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid gap-6 lg:gap-8 md:grid-cols-3 mb-14">
            ...
            >
                <div class="absolute top-0 left-0 h-px w-0 group-hover:w-full transition-all duration-700" style="background-color: {{ $primaryColor }};"></div>
                <div class="w-11 h-11 rounded-full flex items-center justify-center mx-auto mb-6" style="background-color: {{ $primaryColor }}08; border: 1px solid {{ $primaryColor }}12;">
                    <svg class="w-5 h-5" style="color: {{ $primaryColor }};" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                    </svg>
                </div>
                <h3 class="text-xl mb-3" style="color: {{ $headingColor }}; font-family: '{{ $headingFont }}', serif; font-weight: 400;">
                    {{ __('Address') }}
                </h3>
                <p class="text-[14px] leading-[1.7]" style="color: {{ $textColor }};">{{ $address }}</p>
            </div>
            ...
            >
                ...
                <h3 class="text-xl mb-3" style="color: {{ $backgroundColor }}; font-family: '{{ $headingFont }}', serif; font-weight: 400;">
                    {{ __('Call Us') }}
                </h3>
                <a
                ...
                <p class="text-[13px]" style="color: {{ $backgroundColor }}40;">{{ __('Book your appointment') }}</p>
            </div>
            ...
            >
                ...
                <h3 class="text-xl mb-5 text-center" style="color: {{ $headingColor }}; font-family: '{{ $headingFont }}', serif; font-weight: 400;">
                    {{ __('Opening Hours') }}
                </h3>
        ...
        >
            <p class="text-[13px]" style="color: {{ $textColor }}60;">
                {{ __('Walk-ins welcome. Free parking nearby.') }}
            </p>
        </div>
    </div>
</section>
