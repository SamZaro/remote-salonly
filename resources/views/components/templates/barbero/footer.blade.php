{{--
    Template-specifieke footer voor Barbero (Barbershop)
...
<footer id="footer" class="py-20" style="background-color: {{ $backgroundColor }};">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-center gap-4 mb-16">
            <div class="h-px flex-1" style="background: linear-gradient(to right, transparent, {{ $primaryColor }}40);"></div>
            ...
            <div class="h-px flex-1" style="background: linear-gradient(to left, transparent, {{ $primaryColor }}40);"></div>
        </div>
                ...
                <ul class="space-y-3">
                    @foreach($navigation as $item)
                        <li>
                            <a
                                href="#{{ $item['slug'] }}"
                                class="text-gray-400 hover:text-white transition-colors text-sm uppercase tracking-wider"
                            >
                                {{ $item['title'] }}
                            </a>
                        </li>
                ...
                <ul class="space-y-4">
                    @if($address)
                        <li class="flex items-start gap-3">
                            <div class="w-8 h-8 border flex items-center justify-center flex-shrink-0" style="border-color: {{ $primaryColor }}40;">
                    ...
                    @if($phone)
                        <li>
                            <a href="tel:{{ preg_replace('/[^0-9+]/', '', $phone) }}" class="flex items-center gap-3 text-gray-400 hover:text-white transition-colors">
                                <div class="w-8 h-8 border flex items-center justify-center" style="border-color: {{ $primaryColor }}40;">
                                    <svg class="w-4 h-4" style="color: {{ $primaryColor }};" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($email)
                        <li>
                            <a href="mailto:{{ $email }}" class="flex items-center gap-3 text-gray-400 hover:text-white transition-colors">
                                <div class="w-8 h-8 border flex items-center justify-center" style="border-color: {{ $primaryColor }}40;">
                                    <svg class="w-4 h-4" style="color: {{ $primaryColor }};" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex items-center gap-3 mb-8">
                    @if($facebookUrl)
                        <a
                            ...
                            class="w-10 h-10 border flex items-center justify-center transition-all duration-300 hover:bg-white/10"
        ...
        {{-- Bottom Bar --}}
        <div class="pt-8 border-t flex flex-col md:flex-row items-center justify-between gap-4" style="border-color: {{ $primaryColor }}20;">
            <p class="text-gray-500 text-sm">
                {{ $copyright }}
            </p>
            <div class="flex items-center gap-2">
                <span class="text-xs uppercase tracking-widest text-gray-600">Est. {{ date('Y') }}</span>
            </div>
        </div>
    </div>
</footer>
