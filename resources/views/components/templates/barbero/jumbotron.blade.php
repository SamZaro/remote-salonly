{{--
    Template-specifieke jumbotron voor Barbero (Barbershop)
...
<section id="jumbotron" class="relative py-24 lg:py-36 overflow-hidden">
    ...
    <div class="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        @if($subtitle)
            <p
                class="text-xl mb-12 opacity-80 max-w-2xl mx-auto uppercase tracking-widest text-white"
                x-data x-intersect.once="$el.style.opacity = 1; $el.style.transform = 'translateY(0)'"
        ...
        <a
            href="{{ $ctaLink }}"
            class="inline-flex items-center gap-3 px-10 py-5 text-lg font-bold uppercase tracking-wider transition-all duration-300 hover:scale-105"
            style="background-color: {{ $primaryColor }}; color: {{ $secondaryColor }}; opacity: 0; transform: translateY(16px); transition: all 0.8s cubic-bezier(0.22, 1, 0.36, 1) 0.45s;"
            x-data x-intersect.once="$el.style.opacity = 1; $el.style.transform = 'translateY(0)'"
        >
            {{ $ctaText }}
        ...
        <div class="flex items-center justify-center gap-4 mt-12">
            <div class="h-px w-32" style="background: linear-gradient(to right, transparent, {{ $primaryColor }});"></div>
            <div class="w-2 h-2 rotate-45" style="background-color: {{ $primaryColor }};"></div>
            <div class="h-px w-32" style="background: linear-gradient(to left, transparent, {{ $primaryColor }});"></div>
        </div>
    </div>
</section>
