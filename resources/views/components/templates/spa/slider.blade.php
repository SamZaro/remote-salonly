...
@php
    ...
    $subtitle = $content['subtitle'] ?? 'Ontdek pure ontspanning en schoonheidsbehandelingen';
...
>
    ...
    @if($showArrows && $sliderImages->count() > 1)
        <button
            class="absolute left-6 top-1/2 -translate-y-1/2 z-20 w-12 h-12 flex items-center justify-center rounded-full transition-all duration-300 hover:bg-white/20"
        ...
        <button
            class="absolute right-6 top-1/2 -translate-y-1/2 z-20 w-12 h-12 flex items-center justify-center rounded-full transition-all duration-300 hover:bg-white/20"
            style="background-color: rgba(255,255,255,0.1); backdrop-filter: blur(4px); color: #ffffff;"
            @click="nextSlide()"
