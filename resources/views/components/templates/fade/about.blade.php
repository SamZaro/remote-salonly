{{--
    Fade Template: About Section
    Warm cream background — image RIGHT, text LEFT with gold accents
...
@php
    ...
    $description  = $content['description'] ?? __('An authentic barbershop where men feel right at home. We combine traditional techniques with contemporary style.');
    $description2 = $content['description2'] ?? __('Every cut tells a story. Our barbers bring years of expertise, passion, and an eye for detail to every chair.');
    $image        = $section?->getFirstMediaUrl('background') ?: ($content['image'] ?? null);
    $stats        = $content['stats'] ?? [
        ['value' => '8+',  'label' => __('Years experience')],
        ['value' => '5k+', 'label' => __('Satisfied clients')],
        ['value' => '4',   'label' => __('Expert barbers')],
    ];
...
<section id="about" class="relative overflow-hidden" style="background-color: {{ $backgroundColor }};">
    <div class="flex flex-col lg:grid min-h-[80vh]" style="grid-template-columns: 52% 48%;">
        ...
        >
            ...
            {{-- Stats row --}}
            <div class="grid grid-cols-3 gap-6 pt-10 border-t" style="border-color: {{ $primaryColor }}30;">
                @foreach($stats as $stat)
                    <div>
                        <span
                            class="block font-bold leading-none mb-1.5"
                            style="font-family: '{{ $headingFont }}'; font-size: clamp(1.8rem, 3.5vw, 2.8rem); color: {{ $primaryColor }}; letter-spacing: -0.02em;"
                        >
                            {{ $stat['value'] }}
                        </span>
        ...
        <div class="relative order-1 lg:order-2 min-h-[420px] lg:min-h-auto overflow-hidden">
            @if($image)
                ...
                />
                <div class="absolute bottom-0 left-0 w-2 h-32" style="background-color: {{ $primaryColor }};"></div>
                <div class="absolute inset-0 lg:hidden" style="background: linear-gradient(to bottom, transparent 60%, {{ $backgroundColor }});"></div>
            @else
                <div class="absolute inset-0 flex items-center justify-center" style="background-color: {{ $secondaryColor }}08;">
                    <svg class="w-20 h-20 opacity-15" style="color: {{ $primaryColor }};" fill="none" stroke="currentColor" viewBox="0 0 24 24">
