{{--
    Fade Template: Slider Section
    Full-screen carousel — dark overlay, gold accents, bottom content panel
...
@php
    ...
    $subtitle   = $content['subtitle'] ?? __('View our work');
...
>
    ...
    @foreach($sliderImages as $index => $media)
        ...
        >
            ...
            />
            <div class="absolute inset-0" style="background: linear-gradient(to top, rgba(15,15,15,0.8) 0%, rgba(15,15,15,0.35) 40%, transparent 70%);"></div>
        </div>
    @endforeach

    {{-- Slide counter — top right --}}
    <div class="absolute top-10 right-8 lg:right-12 z-20 flex items-baseline gap-2">
        <span class="font-bold text-white" style="font-family: '{{ $headingFont }}'; font-size: 1.5rem; letter-spacing: -0.02em;">
            <span x-text="String(currentSlide + 1).padStart(2, '0')">01</span>
        </span>
        <div class="w-px h-4 self-center" style="background-color: {{ $primaryColor }};"></div>
    ...
    {{-- Content card — warm cream panel anchored bottom --}}
            ...
            >
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-3 mb-4">
                        <div class="w-8 h-0.5 shrink-0" style="background-color: {{ $primaryColor }};"></div>
                        <span class="text-xs font-semibold uppercase tracking-[0.35em]" style="color: {{ $primaryColor }}; font-family: '{{ $bodyFont }}';">{{ __('Barbershop') }}</span>
                    </div>
                    <h2
                        class="font-bold uppercase leading-[0.85] mb-3"
                        style="font-family: '{{ $headingFont }}'; font-size: clamp(2rem, 5vw, 4rem); letter-spacing: -0.02em; color: {{ $secondaryColor }};"
                    >
                        {!! $title !!}
                    </h2>
                    @if($subtitle)
                    ...
                    @if($sliderImages->count() > 1)
                        <div class="flex gap-1">
                            <button
                                ...
                                style="border-color: {{ $primaryColor }}40; color: {{ $secondaryColor }};"
                                onmouseover="this.style.borderColor='{{ $primaryColor }}'; this.style.color='{{ $primaryColor }}'"
                                onmouseout="this.style.borderColor='{{ $primaryColor }}40'; this.style.color='{{ $secondaryColor }}'"
                            ...
                            <button
                                ...
                                onmouseout="this.style.backgroundColor='{{ $secondaryColor }}'; this.style.color='{{ $primaryColor }}'"
                                aria-label="{{ __('Next') }}"
                            >
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
    ...
    <div class="relative z-10 w-full">
        <div class="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12 pb-8">
            <div class="p-10 lg:p-14" style="background-color: {{ $backgroundColor }};">
                <div class="flex items-center gap-3 mb-4">
                    <div class="w-8 h-0.5" style="background-color: {{ $primaryColor }};"></div>
                    <span class="text-xs font-semibold uppercase tracking-[0.35em]" style="color: {{ $primaryColor }}; font-family: '{{ $bodyFont }}';">{{ __('Barbershop') }}</span>
                </div>
                <h2 class="font-bold uppercase leading-[0.85] mb-4" style="font-family: '{{ $headingFont }}'; font-size: clamp(2rem, 5vw, 4rem); letter-spacing: -0.02em; color: {{ $secondaryColor }};">
                    {!! $title !!}
                </h2>
                @if($subtitle)
                ...
                @endif
                @if($ctaText)
                    <a href="{{ $ctaLink }}" class="inline-flex items-center gap-2 px-7 py-4 font-semibold uppercase tracking-widest text-sm hover:opacity-90" style="background-color: {{ $primaryColor }}; color: {{ $secondaryColor }}; font-family: '{{ $bodyFont }}';">
                        {{ $ctaText }}
                    </a>
                @endif
            </div>
