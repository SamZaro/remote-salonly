{{--
    Fade Template: Features Section
    Dark section — gold icon blocks, bordered feature cards with hover
...
@php
    ...
    $subtitle = $content['subtitle'] ?? __('Our strengths');
    $items    = $content['items'] ?? [
        ['title' => __('Master Barbers'),       'description' => __('Skilled craftsmen with years of experience in classic and modern techniques.'), 'icon' => 'scissors'],
        ['title' => __('Personal Service'),     'description' => __('Every client gets a personal consultation for the perfect result.'), 'icon' => 'chat'],
        ['title' => __('Premium Products'),     'description' => __('We exclusively use professional grooming products from top brands.'), 'icon' => 'sparkles'],
        ['title' => __('Relaxed Vibe'),         'description' => __('A welcoming space where you can unwind and feel at home.'), 'icon' => 'heart'],
    ];
...
<section id="features" class="py-20 lg:py-28" style="background-color: {{ $secondaryColor }};">
    <div class="mx-auto max-w-7xl px-6 sm:px-8 lg:px-12">
        ...
        {{-- Section header --}}
        <div
            class="mb-16 lg:mb-20 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6"
        ...
        >
            ...
            <p class="text-base max-w-xs font-light" style="color: rgba(255,255,255,0.45); font-family: '{{ $bodyFont }}';">
                {{ $subtitle }}
            </p>
        </div>

        {{-- Features grid — bordered cards with gold accent --}}
        <div class="grid gap-0 border sm:grid-cols-2 lg:grid-cols-4" style="border-color: rgba(200,184,138,0.15);">
            @foreach($items as $item)
                <div
                    class="group relative p-8 lg:p-10 flex flex-col border-b sm:border-b-0 sm:border-r transition-all duration-300"
                    style="
                        background-color: transparent;
                        border-color: rgba(200,184,138,0.15);
                        opacity: 0;
                        transform: translateY(20px);
                        transition: opacity 0.8s cubic-bezier(0.22, 1, 0.36, 1) {{ $loop->index * 0.1 }}s, transform 0.8s cubic-bezier(0.22, 1, 0.36, 1) {{ $loop->index * 0.1 }}s, border-color 0.3s ease;
                    "
                    x-data x-intersect.once="$el.style.opacity = 1; $el.style.transform = 'translateY(0)'"
                ...
                >
                    {{-- Gold icon block --}}
                    <div class="mb-8">
                        <div class="w-12 h-12 flex items-center justify-center" style="background-color: {{ $primaryColor }};">
                    ...
                    {{-- Title --}}
                    <h3
                        class="text-base font-bold uppercase tracking-wide mb-3"
                    ...
                    {{-- Description --}}
                    <p class="text-sm leading-relaxed font-light mt-auto" style="color: rgba(255,255,255,0.4); font-family: '{{ $bodyFont }}';">
                        {{ $item['description'] }}
                    </p>

                    {{-- Bottom gold line on hover --}}
                    <div class="absolute bottom-0 left-0 w-0 h-0.5 group-hover:w-full transition-all duration-500" style="background-color: {{ $primaryColor }};"></div>
                </div>
            @endforeach
        </div>
    </div>
