        ...
        @if(count($items) > 0)
            <div class="grid gap-8 {{ $gridCols }}" x-intersect="$el.classList.add('fadeInUp')">
                @foreach($items as $item)
                    <div
                        class="text-center p-8 rounded-sm transition-all duration-300 hover:-translate-y-1"
                        style="background-color: {{ $primaryColor }}10;"
                    >
                        {{-- Icon --}}
                        <div
                            class="w-14 h-14 rounded-sm flex items-center justify-center mx-auto mb-6"
                        ...
                        <h3 class="text-lg font-bold mb-3" style="color: #ffffff;">
                            {{ $item['title'] ?? '' }}
                        </h3>
